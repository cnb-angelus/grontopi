...
from models.api_models import EntityDescription, EntityListWithLabels
from models.ontology_models import ClassURI, EntityURI
...
graph = SPARQLAccess(
    query_endpoint=cfg.sparql_endpoint,
...
@router.get(
    ...
    tags=["Vocabularies"],
    description=(
        "Catalogs, Relations and Classes (see description of " "respective outputs)"
    ),
)
...
@router.get("/entities/by_id", tags=["Statements"], response_model=EntityDescription)
async def entities_by_id(
    entity_id: EntityURI = exent,
    lang: str = deflang,
    user_info: str = Depends(user_invalidator()),
):
    res = await graph.fetch_entities_from_list_of_ids(
        entitylist=[entity_id], onto=onto, lang=lang
    )
    ...
    if isinstance(res, dict):
        if not any(
            (
                res["data_properties"],
                res["object_properties"],
                res["inverse_properties"],
            )
        ):
            raise HTTPException(status_code=404, detail="Entity without properties")
        return res
...
@router.post(
    "/entities/by_ids", tags=["Statements"], response_model=List[EntityDescription]
)
async def entities_by_ids(
    entity_ids: List[EntityURI] = exents,
    lang: str = deflang,
    user_info: str = Depends(user_invalidator()),
):
    res = await graph.fetch_entities_from_list_of_ids(
        entitylist=entity_ids, onto=onto, lang=lang
    )
...
async def entities_labels(
    class_id: ClassURI = excls,
    start: int = 0,
    per_page: int = 100,
    lang: str = deflang,
    prefix: str = "",
    ...
    res = await graph.fetch_entities_of_classes(
        class_id=class_id, start=start, per_page=per_page, lang=lang, prefix=prefix
    )
    return res


@router.get("/entitites/connected_to", tags=["Statements"])
async def entities_connected_to(
    central_entity: EntityURI = exent,
    lang: str = deflang,
    user_info: str = Depends(user_invalidator()),
):
    chk = await graph.check_existence_of_entities([central_entity], onto_config=onto)
    if len(chk) > 0:
        raise HTTPException(
            status_code=400,
            detail={"message": "Entity does not exist", "entities": chk},
        )
    res = await graph.fetch_entities_around(
        entity_id=central_entity, lang=lang, onto_config=onto
    )
